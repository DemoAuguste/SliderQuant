...
**Weight-only 量化**（与论文一致）：activation 全部保持 bf16，只有 decoder 层的投影 Linear 被量化。

**覆盖范围**（`build_groups(mode="split")`，72 组 = 每层 attn/mlp 两组）：

| 类别 | 是否量化 | 分组/数量 |
|---|---|---|
| self_attn q/k/v/o_proj | ✅ | 36 组 |
| mlp gate/up/down_proj | ✅ | 36 组 |
| embed_tokens / lm_head | ❌ | 保持 bf16 |
| 各 RMSNorm | ❌ | 保持 bf16 |

被量化：**252 个 Linear，6.946G 参数**；未量化：embed + lm_head + norms（≈1.24G 参数，2.49 GB）。

**完整模型大小（实测 safetensors，Qwen3-8B bf16）**：

| 部分 | 参数量 | 量化前(bf16) | 量化后 |
|---|---|---|---|
| 被量化（5.554 bits/参数） | 6.946G | 13.89 GB | 4.82 GB |
| 未量化（bf16 保持） | 1.24G | 2.49 GB | 2.49 GB |
| **完整模型** | **8.19G** | **16.38 GB** | **≈ 7.31 GB** |

- **压缩率 = 16.38 / 7.31 ≈ 2.24×**；被量化部分本身 13.89 → 4.82 GB ≈ 2.88×
- 位宽 5.554 bits 为纯权重位宽（未计入 scale/zero-point 元数据），与论文"含所有 overhead"口径略有差异
